...
'imgs/bird_pink.png','imgs/bird_yellow.png','imgs/bird_orange.png','imgs/bird_silver.png'];
        ...
        FB.Collides = function (bird, pipe) {
            ...
            if (pipe.coin && bird.vx > pipe.centerX + pipe.w / 2 - 5) {
                pipe.coin = false;
                allScores[FB.currentPlayerIndex] += 1;
                FB.score.coins += 1;
  ...
  function gotStartMatch(params) {
    ...
    resizeGameAreaService.setWidthToHeight(0.5);
     ...
     allFB[yourPlayerIndex] = FB1;
     for(var i = 0; i<allFB[yourPlayerIndex].entities.length; i++){
        if(allFB[yourPlayerIndex].entities[i].type === 'bird' && !contains(allBirds,allFB[yourPlayerIndex].entities[i])){
            allBirds.push(allFB[yourPlayerIndex].entities[i]);
        }
     }
   ...
   //     firstStart = false;
    ...
    function sendMessage(isReliable) {
        ...
        for(; i > len; i--){
            birds[j] = f.entities[i];
            j++;
        }
        var birdsInfo = [];
        for(var i = 0;i < birds.length; i++){
            if(birds[i].playerIndex === yourPlayerIndex){
                birdsInfo.push({
                
                    gravity : birds[i].gravity,             
                    width :birds[i].width,
                    height : birds[i].height,
                    ix : birds[i].ix,
                    iy : birds[i].iy,
                    fr : birds[i].fr,
                    vy : birds[i].vy,
                    ...
                    play : birds[i].play,
                    jump : birds[i].jump,
                    rotation : birds[i].rotation,
                    type : birds[i].type,
                    playerIndex : birds[i].playerIndex
                    });
                break;
            }
        }
    ...
    isGameOngoing = false;
     ...
     function contains(arr,obj){
        for(var i=0; i<arr.length; i++){
            if(arr[i].playerIndex === obj.playerIndex)
                return true;
        }
        return false;
     }
                ...
                for(var i=0; i < playersInfo.length; i++){
                    ...
                    if(i!=yourPlayerIndex)
                    {
                       ...
                       FB.entities.push(FB.bird[i]);
                        if(!contains(allBirds,FB.bird[i]))
                            allBirds.push(FB.bird[i]);
                    }
                }
                    FB.bird[yourPlayerIndex] = new FB.Bird(yourPlayerIndex);    
                    FB.entities.push(FB.bird[yourPlayerIndex]);
                    if(!contains(allBirds,FB.bird[yourPlayerIndex]))
                        allBirds.push(FB.bird[yourPlayerIndex]);
                for(var n=0;n<10;n++){
                    var img = new Image();
            ...
            this.update1 = function() {
                    ...
                    for(;i>len;i--){
                         if(FB.entities[i].playerIndex != FB.currentPlayerIndex){
                            for(var j = 0; j < allBirds.length; j++){
                            
                                if(FB.entities[i].playerIndex === allBirds[j].playerIndex ) {
                                    FB.entities[i] = allBirds[j];
                                    break;
                                }
                            }
                        }
