...
'imgs/bird_pink.png','imgs/bird_yellow.png','imgs/bird_orange.png','imgs/bird_silver.png']
       ...
       // var FB = null;
        ...
        var FB = function () {
           ...
           // ctx= null,
            ...
            this.init = function (canvas) {
             ...
             //   this.resize();
                this.canvas.changeState("Splash");
            ...
            this.changeState = function(state) {                   
                ...
                if(state === 'Splash'){
                    this.game = new this.Splash();
                }
                else if(state === 'Play'){
                    this.game = new this.Play();   
                }
                else{
                    this.game = new this.GameOver();      
                }
                this.game.init();
            }
            this.Draw = {
        ...
        this.Splash = function(){
            ...
            this.init = function(){
                ...
                //Add entities
                that.entities.push(new that.Cloud(30, ~~ (randomService.random(randomIndex) * that.HEIGHT / 2)));
                that.entities.push(new that.Cloud(130, ~~ (randomService.random(randomIndex) * that.HEIGHT / 2)));
                that.entities.push(new that.Cloud(230, ~~ (randomService.random(randomIndex) * that.HEIGHT / 2)));
                for (var i = 0; i < 2; i += 1) {
                    that.entities.push(new that.BottomBar(that.WIDTH * i, that.HEIGHT - 100, that.WIDTH));
                }
                that.entities.push(new that.Tree(~~(randomService.random(randomIndex) * that.WIDTH), that.HEIGHT - 160));
                that.entities.push(new that.Tree(~~(randomService.random(randomIndex) * that.WIDTH + 50), that.HEIGHT - 160));
                that.entities.push(new that.Tree(~~(randomService.random(randomIndex) * that.WIDTH + 100), that.HEIGHT - 160));
            }
            ...
            this.update = function(){
                ...
                if (that.Input.tapped) {
                    //FB.changeState('Play');
                    that.Input.tapped = false;
                }
            }
    ...
    //  console.log(secondsFromStart);
                that.Draw.Image(this.banner,66,100);
    ...
    that.Draw.text(msg,canvasWidth / 4 - 60, canvasHeight / 4 - 30, 20, yourColor );

      that.Draw.text("" + secondsToReallyStart,canvasWidth / 2 , canvasHeight / 2 -40, 40, yourColor);
